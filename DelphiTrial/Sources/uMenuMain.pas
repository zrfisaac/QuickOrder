...
type
  TMenuMain = class(TdxFluentDesignForm)
    ...
    procedure dxNavBarAboutClick(Sender: TObject);
  private
    FMenu: TForm;
  public
    procedure Clean;
...
procedure TMenuMain.dxFluentDesignFormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  if Assigned(MenuOrder) then
  begin
    with (MenuOrder) do
    begin
      pnFooter.SetFocus;
      edClient.ClearSelection;
      edProduct.ClearSelection;
    end;
  end;
  if Assigned(FMenu) then
    TModelForm(FMenu).pnBack.Parent := TModelForm(FMenu);
  if Assigned(DataMain) then
  begin
    with (DataMain) do
    begin
...
begin
  MenuConfig.Connnect;
  if (MenuConfig.Status) then
    Self.Menu(TMenuOrder, MenuOrder)
  else
    Self.Menu(TMenuConfig, MenuConfig);
end;

procedure TMenuMain.dxNavBarAboutClick(Sender: TObject);
begin
  TMenuAbout.Start;
end;
